...
export const useDataStore = defineStore('data', {
    ...
    actions: {
        randomJourney() {
            while(this.journey.length < 9) {
                const randomIndex = Math.floor(Math.random() * this.datas.length);
                if(!this.journey.includes(this.datas[randomIndex])) {
                    this.journey.push(this.datas[randomIndex])
                }
            }
        }
    }
